...
| `credentials.<b64-email>.enc` | Encrypted OAuth refresh token + its OAuth client (self-contained per account) | `PERSONAL_GWS_CREDENTIALS_<ORG>_ENC_B64` (stored base64; `<ORG>` = uppercase first label of the email domain, e.g. `BENALI`, `DEMANDCAST`) |
...
- **Suffix derivation is domain-based.** `khalil@benali.com` → `BENALI`, `khalil@demandcast.co` → `DEMANDCAST`. Two accounts on the *same* domain would collide — add a disambiguating scheme before that ever happens.
